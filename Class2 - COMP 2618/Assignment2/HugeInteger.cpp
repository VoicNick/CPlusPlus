...
{
	// Voicu's implementation:
	return ((*this).hugeInt == op.hugeInt) ;
}
...
HugeInteger::operator double(void)const
{
	...
	unsigned short currentElement = hugeInt.size() - 1;
	long double result = 0;
	long double powerOfTen = 1.0;

	

	while (currentElement >i )
	{
		result = result + this->hugeInt[currentElement] * powerOfTen;
		currentElement--;
		powerOfTen *= 10;
	}
...
// PLEASE NOTE:  The remainder are functions, not member functions!!!
